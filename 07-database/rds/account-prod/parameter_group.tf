...
resource "aws_db_parameter_group" "account_aurora_pg" {
  name = "account-aurora-${data.terraform_remote_state.vpc.outputs.shard_id}-pg"
...
resource "aws_rds_cluster_parameter_group" "dayone_aurora_cluster_pg" {
  name = "account-aurora-${data.terraform_remote_state.vpc.outputs.shard_id}-cluster-pg"
